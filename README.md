...
## Folder Structure (Under Development)
...
```
$ git clone https://github.com/trioXech/python-core.git
````
...
Now, this is where it gets messy. Don't you ever work on the master branch. Let's refresh your memories on how to make branches.

```
$ git checkout -b branchName
```

> Tip: Each branch should be made based on each topics or problem, not page. For example, some branches could be: multilineString, 2dList, listComprehension etc.
...
Double check to make sure you are up to date with master branch. But first, you must commit your changes on your branch. Before applying outside changes, you should get your own work in good shape and committed locally, so it will not be clobbered if there are conflicts.

```
# on you branch, git add
$ git commit -m "blah"
$ git checkout master
$ git pull origin master
```
...
The difference from before is simply the clone :
